...
import threading
from autoenc_load import load_model  # Import the load_model function from complex_autoencoder.py
...
model = load_model('/Users/prasanna/Desktop/Hack2Future/complex_autoencoder_model.pth')  # Load your trained model here
...
def run_anomaly_detection():
    ...
    while True:
        ...
        if len(streaming_data_buffer) >= input_length + output_length:
            ...
            with torch.no_grad():
                # Get the model's reconstruction (prediction)
                prediction = model(current_window)
                # The target is the same as the input to the autoencoder for reconstruction
